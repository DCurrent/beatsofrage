...
static int loadwave(char *filename, char *packname, samplestruct *buf, unsigned int maxsize){
	...
	if((buf->sampleptr=(char*)malloc(maxsize*mulbytes+8))==NULL){
		closepackfile(handle);
		return 0;
	}
...
void sound_close_music(){
	...
	if(adpcm_inbuf) free(adpcm_inbuf);
	adpcm_inbuf = NULL;

	for(i=0; i<MUSIC_NUM_BUFFERS; i++){
		if(musicchannel.buf[i]) free(musicchannel.buf[i]);
...
int sound_open_music(char * filename, char * packname, int defaultvolume, int loop){
	...
	adpcm_inbuf = (char*)malloc(MUSIC_BUF_SIZE / 2);
	if(adpcm_inbuf==NULL){
		sound_close_music();
		return 0;
	}
	...
	for(i=0; i<MUSIC_NUM_BUFFERS; i++){
		musicchannel.buf[i] = (short *)malloc(MUSIC_BUF_SIZE*sizeof(short));
...
int sound_init(int channels){
	...
	if((mixbuf = (unsigned int *)malloc(MIXBUF_SIZE)) == NULL){
		SB_exit();
		return 0;
	}
